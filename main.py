...
app = Flask(__name__)
app.secret_key = 'super_tajny_klic_pro_session_zabezpeceni'  # Nutné pro fungování session

ACCESS_CODE = "CESKYtransmit75"
MAX_DISPLAYS = 6
...
<head>
    ...
    <title>Přihlášení - Build Logic Hub</title>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700&display=swap" rel="stylesheet">
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { font-family: 'Inter', sans-serif; background-color: #0b0f19; color: #c9d1d9; display: flex; justify-content: center; align-items: center; min-height: 100vh; }
        .login-card { background: #161b22; border: 1px solid #30363d; border-radius: 12px; padding: 30px; width: 100%; max-width: 400px; box-shadow: 0 8px 24px rgba(0,0,0,0.5); text-align: center; }
        h1 { font-size: 22px; color: #58a6ff; margin-bottom: 10px; }
        p { color: #8b949e; font-size: 14px; margin-bottom: 20px; }
        input[type=password] { width: 100%; padding: 12px; background: #0d1117; border: 1px solid #30363d; border-radius: 6px; color: #f0f6fc; font-size: 15px; margin-bottom: 15px; text-align: center; }
        input[type=password]:focus { border-color: #58a6ff; outline: none; }
        button { width: 100%; padding: 12px; border-radius: 6px; border: none; font-weight: 600; font-size: 14px; cursor: pointer; background: #238636; color: #ffffff; }
        button:hover { background: #2ea043; }
        .error { color: #f85149; font-size: 13px; margin-bottom: 15px; }
    </style>
</head>
<body>
    <div class="login-card">
        <h1>🔒 Přístup zablokován</h1>
        <p>Zadejte přístupový kód pro ovládání panelu</p>
        {% if error %}
            <div class="error">{{ error }}</div>
        {% endif %}
        <form method="POST" action="/login">
            <input type="password" name="code" placeholder="Přístupový kód" required autofocus>
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .logout-btn { position: absolute; right: 0; top: 0; background: #21262d; border: 1px solid #30363d; color: #c9d1d9; padding: 6px 12px; border-radius: 6px; text-decoration: none; font-size: 12px; }
        .logout-btn:hover { background: #30363d; color: #fff; }
...
<body>
    <div class="container">
        <header>
            <a href="/logout" class="logout-btn">🔒 Odhlásit</a>
...
def login():
    if request.method == 'POST':
        ...
        if entered_code == ACCESS_CODE:
            ...
            return render_template_string(LOGIN_TEMPLATE, error="Neplatný kód!")
...
# --- HLAVNÍ WEBOVÉ TRASY (CHRÁNĚNÉ HESLEM) ---
@app.route('/')
def home():
...
# --- NECHRÁNĚNÉ ENDPOINTY PRO ROBLOX (GET / POST API) ---
